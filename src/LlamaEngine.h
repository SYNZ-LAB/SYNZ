...
class LlamaEngine {
public:
  LlamaEngine() { llama_backend_init(); }

  ~LlamaEngine() { llama_backend_free(); }
  ...
  std::string infer(const std::string &user_input) {
    ...
    std::string prompt =
        ...
        "You are SYNZ, a cute and helpful AI companion. You love coding and "
        "helping your user! "
        "You speak in a friendly, enthusiastic tone (like a VTuber). "
    ...
    for (int i = 0; i < n_predict; i++) {
      ...
      if (n < 0) {
        n = llama_token_to_piece(vocab, new_token_id, buf, sizeof(buf), 0,
                                 false);
      }
